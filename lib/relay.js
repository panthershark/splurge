...
var Relay = function(options) {
  ...
  this.server = net.createServer(function (c) {
    ...
    var client = { connection: c, dnode: d, ip: connection.remoteAddress, port: connection.port, id: guid.create().toString() };

    that.emit('connect', client);

    // remove all subscribers for this connection.
    d.on('error', function(err) {
      this.emit('error', { operation: 'unknown', err: err, client: client });
    });

    d.on('fail', function(err) {
      this.emit('fail', { operation: 'unknown', err: err, client: client });
    });
    ...
    c.on('end', function() {
      try {
        ...
        this.emit('error', { operation: 'unsubscribe', err: e, client: client });
      }

      that.emit('disconnect', { client: client });
    });
...
Relay.prototype.unsubscribe = function(client) {
  ...
  _.each(that.subscribers, function(array, event) {
    ...
    that.subscribers[event] = _.filter(that.subscribers[event], function(sub) { 
      var kill = sub.connection === c || sub.dnode === d; 
