...
var Relay = function(options) {
  ...
  this.server = net.createServer(function (c) {
    var d = dnode({
      ...
      subscribe: function(event, cb) {
        ...
        try {
          ...
          that.subscribers[event].push({ connection: c, dnode: d });
        } catch (e) {
          err = e;
        ...
        if (!err) {
          that.emit('subscribe', { connection: c, event: event });
        }
    ...
    d.on('error', function(err) {
      this.emit('error', { operation: 'unknown', err: err });
    });
...
Relay.prototype.unsubscribe = function(subscription) {
  var c = subscription.connection,
      d = subscription.dnode,
      that = this;

  _.each(that.subscribers, function(array, event) {
    that.subscribers[event] = _.filter(that.subscribers[event], function(sub) { return sub.connection === c || sub.dnode === d; });
  });
};
